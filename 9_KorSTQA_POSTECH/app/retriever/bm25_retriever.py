...
def measure_retriever_accuracy(retriever, retriever_test_data, n:int, doc_to_document_passages):
    ...
    retrieved_results = []
    for datapoint in retriever_test_data:
...
def evaluate_retriever(retriever, retriever_test_data, doc_to_document_passages):
    ...
    그리고 retrieved된 passage들을 json파일로 저장함 
...
class BM25Retriever(object):
    def __init__(self, passages_list:List,  do_normalize:bool):
    ...
    def create_index(self):
        """
        Create index using passages_list
        """
        print("Creating index..")
        
        if self.do_normalize:
            tokenized_corpus = [simple_tokenizer(doc) for doc in self.corpus]
            self.index = BM25Okapi(tokenized_corpus)
        else:
            tokenized_corpus = [normalized_tokenizer(doc) for doc in self.corpus]
            self.index = BM25Okapi(tokenized_corpus)
        
        print("Index created!")
    
    def query_index(self, query:str, top_n:int):
        """ 
        생성된 index에서, top_n document를 return한다.
        """
        if self.do_normalize:
            tokenized_query = simple_tokenizer(query)
        else:
            tokenized_query = normalized_tokenizer(query)

        matched_documents = self.index.get_top_n(tokenized_query, self.corpus, n=top_n)
...
def main(args):
    ...
    retriever = BM25Retriever(mypassage_list, do_normalize=args.do_normalize)
